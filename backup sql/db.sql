...
-- Dumping data for table app_tabungan.data_nasabah: ~5 rows (approximately)
INSERT INTO `data_nasabah` (`id_nasabah`, `nama`, `no_rekening`, `id_jenjang`, `saldo`, `jk`) VALUES
	(8, 'Azmi', '001', 1, 50000, 'L'),
	(9, 'Lani', '002', 1, 75000, 'P'),
	(10, 'Sila', '003', 2, 120000, 'P'),
	(13, 'dini', '001', 3, 50000, '');
...
-- Dumping data for table app_tabungan.jenjang: ~2 rows (approximately)
...
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `view_nasabah` AS select `data_nasabah`.`id_jenjang` AS `id_jenjang`,`data_nasabah`.`id_nasabah` AS `id_nasabah`,`data_nasabah`.`no_rekening` AS `no_rekening`,`data_nasabah`.`nama` AS `nama`,`data_nasabah`.`saldo` AS `saldo`,`data_nasabah`.`jk` AS `jk`,`jenjang`.`nama_jenjang` AS `nama_jenjang` from (`data_nasabah` join `jenjang` on((`data_nasabah`.`id_jenjang` = `jenjang`.`id_jenjang`)));

-- Dumping structure for view app_tabungan.view_transaksi
-- Removing temporary table and create final VIEW structure
DROP TABLE IF EXISTS `view_transaksi`;
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `view_transaksi` AS select `transaksi`.`id_nasabah` AS `id_nasabah`,`transaksi`.`id_transaksi` AS `id_transaksi`,`transaksi`.`tanggal` AS `tanggal`,`transaksi`.`kode_tr` AS `kode_tr`,`transaksi`.`setor` AS `setor`,`transaksi`.`tarik` AS `tarik`,`data_nasabah`.`nama` AS `nama`,`data_nasabah`.`no_rekening` AS `no_rekening`,`data_nasabah`.`id_jenjang` AS `id_jenjang`,`data_nasabah`.`saldo` AS `saldo`,`data_nasabah`.`jk` AS `jk` from (`transaksi` join `data_nasabah` on((`transaksi`.`id_nasabah` = `data_nasabah`.`id_nasabah`)));
